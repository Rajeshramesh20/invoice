...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function calculateRow() {
        ...
        const isInclusive = row.querySelector(".inclusive-checkbox")?.checked;
    ...
    function calculateTotals() {
        let netTotal = 0, gstTotal = 0, grandTotal = 0;

        document.querySelectorAll("#itemTableBody tr").forEach(row => {
            if (!row.id || row.id !== "addButtonRow") {
    ...
    function createRow() {
        ...
        row.innerHTML = `
            ...
            <td>
                <input type="number" class="form-control unit-price" placeholder="Unit Price" required>
                <div class="form-check mt-1">
                    <input type="checkbox" class="form-check-input inclusive-checkbox">
                    <label class="form-check-label small">Inclusive</label>
                </div>
            </td>
        ...
        `;
        tableBody.insertBefore(row, document.getElementById("addButtonRow"));

        row.querySelector(".quantity").addEventListener("input", calculateRow);
        row.querySelector(".unit-price").addEventListener("input", calculateRow);
        row.querySelector(".gst").addEventListener("input", calculateRow);
        row.querySelector(".inclusive-checkbox").addEventListener("change", calculateRow);
    ...
    document.getElementById('invoiceForm').addEventListener('submit', function (e) {
        ...
        document.querySelectorAll('#itemTableBody tr:not(#addButtonRow)').forEach(row => {
            const itemName = row.querySelector('.item-name')?.value;
            const qty = row.querySelector('.quantity')?.value;
            const price = row.querySelector('.unit-price')?.value;
            const gst = row.querySelector('.gst')?.value;
            const inclusive = row.querySelector('.inclusive-checkbox')?.checked;
            ...
            if (itemName && qty && price && gst) {
                formData.items.push({
                    ...
                    is_inclusive_price: inclusive
                });
            }
        });

        console.log("Sending Invoice Data:", formData); // ✅ For debug
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    ...
                    alert('You are unauthorized to create invoices');
                    window.location.href = "/api/invoice/list";
                    ...
                    if (response.errors) {
                        for (let key in response.errors) {
                            ...
                            if (errorElement) {
                                errorElement.innerText = response.errors[key].join(', ');
                            }
                        }
                    }
                } else {
